...
class Account:
    ...
    def get_balance(self):
        if self.__is_frozen or self.__closed_account:
            return "Account is frozen or closed. You can't transact."
        balance = 0
    ...
    def deposit(self, amount):
        if self.__is_frozen or self.__closed_account:
            return "Account is frozen or closed. You can't deposit."
        if amount > 0:
    ...
    def withdraw(self, amount):
        if self.__is_frozen or self.__closed_account:
            return "Account is frozen or closed. You can't withdraw."
        balance = self.get_balance()
        if isinstance(balance, str):
            return balance
        if balance - amount < self.__min_balance:
            return f"Sorry, you need a minimum of {self.__min_balance} in your account."
        if amount > 0:
    ...
    def transfer_fund(self, amount, recipient):
        ...
        if self.__is_frozen or self.__closed_account:
            return "Account is frozen or closed. You can't transfer."
        if amount < 0:
        ...
        if isinstance(balance, str):
            return balance
        if balance - amount < self.__min_balance:
            return f"Sorry, you can not transfer {amount}, your balance is {balance}"
        else:
            ...
            return f"You have successfully transferred {amount} to {recipient.name}. Your new balance is {self_balance}."
    def request_loan(self, amount):
        if self.__is_frozen or self.__closed_account:
            return "Account is frozen or closed. You can't request a loan."
        ...
        if amount <= loan_limit:
            ...
            return f"Loan of {amount} approved. Your new balance is {self.get_balance()}, your loan balance is {self.__loan}."
        else:
            return f"Invalid loan amount. Your request should be less than {loan_limit}."
    def repay_loan(self, amount):
        if self.__is_frozen or self.__closed_account:
            return "Account is frozen or closed. You can't repay loan."
        balance = self.get_balance()
        if self.__loan == 0:
        ...
        if amount <= self.__loan:
            ...
            return f"You have repaid {amount}, you are left with {self.__loan} of loan."
        else:
    ...
    def account_statement(self):
        ...
        for trx in self.__transactions:
            print(trx)
        if self.__loan != 0:
            print(f"Outstanding loan: {self.__loan}.")
    def interest(self):
        balance = self.get_balance()
        if isinstance(balance, str):
            return balance
        ...
        return f"Your balance after interest is {self.get_balance()}."
